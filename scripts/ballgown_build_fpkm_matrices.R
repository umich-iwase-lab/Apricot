...
#option_list = list(
#    make_option('--input_dir', type='character', help='[Required] Path to ballgown directory'),
#    make_option('--output_dir', type='character', help='[Required] Path to output directory')
#)
...
# Match the position in gene_fpkms to the position in iso_fpkms, which contains gene_name information
indices <- match(rownames(gene_fpkms), iso_fpkms$gene_id)

# Pull out gene_name from iso_fpkms
gene_names_for_result <- iso_fpkms$gene_name[indices]

# Retain the id column and gene_name column
gene_fpkms = cbind(id = rownames(gene_fpkms), gene_name=gene_names_for_result, data.frame(gene_fpkms))
...
gtf_dict = iso_fpkms[, c('t_id', 'gene_id','gene_name', 't_name', 'chr', 'start', 'end', 'strand')]

# Retain identifying information for the transcripts
iso_col_keep = c('t_id', 'gene_id', 'gene_name','t_name', 'chr', 'start', 'end', 'strand', grep('FPKM', colnames(iso_fpkms), value = T))
